...
# Fetch the playlist data, with optional ARL token, and handle pagination
def get_playlist_data(arl_token, playlist_id, callback):
    headers = {}
    if arl_token:  # Only include Authorization header if ARL token is provided
        headers["Authorization"] = f"Bearer {arl_token}"
    
...
        response = requests.get(playlist_url, headers=headers if headers else None)
        
...
        messagebox.showerror("Error", "Please fill Playlist ID and File Path fields!")
        return
...
tk.Label(root, text="ARL Token (optional for public playlists):").pack(pady=5)
